...
ENV HOSTNAME mx.qiku.info
ENV DOCKER_LDAP_DN dc=qiku,dc=info
...
RUN wget -O - --no-check-certificate \
  ...
  && sed -i 's/dc=qiku,dc=info/'"$DOCKER_LDAP_DN"'/' \
...
RUN cp /opt/iredmail/conf/ldap_init.ldif ldifs/00_ldap_init.ldif \
  ...
  && for f in ldifs/*.ldif; \
  do \
    ( ldapadd -D 'cn=Manager,'"$DOCKER_LDAP_DN" -w zhf2009 -f "$f" || \
    ldapmodify -v -D 'cn=Manager,'"$DOCKER_LDAP_DN" -w zhf2009 -f "$f" ); \
  done
